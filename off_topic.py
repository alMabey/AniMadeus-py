...
class OffTopicCog(commands.Cog):
    ...
    async def check_message(self, message):
        if message.channel.id == bot_data.CHANNEL_IDS['off-topic'] and not message.author.bot:
            ...
            elif ('no' in message.content.lower()) and (('gf' in message.content.lower()) or ('bf' in message.content.lower()) or ('boy' in message.content.lower()) or ('girl' in message.content.lower())):
                ctx = await self.bot.get_context(message)
                await ctx.reply('No! Bad! https://i.imgur.com/PLYtGqI.jpg')                                                                       
